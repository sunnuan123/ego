...
<div id="itemEditWindow" class="easyui-window" title="编辑商品" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/goods/edit'" style="width:80%;height:80%;padding:10px;">
</div>
<script>
    ...
    var toolbar = [{
        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			...
        			// 加载商品描述
        			$.getJSON('/rest/goods/query/goods/desc/'+data.id,function(_data){
        				if(_data.status == 200){
        			...
        			//加载商品规格
        			$.getJSON('/rest/goods/param/goods/query/'+data.id,function(_data){
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
        	    	$.post("/goods/delete",params, function(res){
            			if(res.status == 200){
