	...
	_t.dom.addEventListener(clickEvent,function(e){
		...
		$.ajax({
			...
			success:function(res){
				if(200==res.status && res.message>0){
					_t.sum++;
					amount.innerHTML = res.message/100;
					_t.amount = res.message/100;
					target.appendChild(amount);
					target.className = 'cell block_click'+s;
					_t.state = false;
					clearInterval(_t.timer);
					_t.end();
				}else if(200==res.status && res.message==-1){
					alert("您已抢过该红包");
				}else{
					alert("红包已抢完");
				}
...
Block.prototype.end = function(){
	...
	if( !document.getElementById("againMask") ){
		...
		// againMask.innerHTML =
		'<div id="gove" ><h2 id="againSum">哇塞!你只抢到了'+_t.amount+'元红包</h2><h1>难道是大神没附体!</h1><span id="againStart"><img src="/img/e1.png"></span><span id="againStart_two"><img src="/img/e2.png"></span><div>';
	...
	/***************判断****************/
	againMask.innerHTML =
			'<div id="gove" ><h2 id="againSum">哇塞!你抢到了'+_t.amount+'元红包</h2><span id="againStart"><img src="/img/e1.png"></span><span id="againStart_two"><img src="/img/e2.png"></span><div>';
